...
def create_duplex_simplex_dataframe(simplex_dataframe, create_duplex_simplex_dataframe):
    ...
    try:
        df_s["t_total_count_fragment_simplex"] = df_ds["t_ref_count_fragment_simplex"] + df_ds["t_alt_count_fragment_simplex"]
        logger.debug("genotype:variants:small_variants::create_duplex_simplex_dataframe:: Successfully generated t_total_count_fragment_simplex column")
    except:
        e = sys.exc_info()[0]
        logger.error("genotype:variants:small_variants::create_duplex_simplex_dataframe:: Could not generate t_total_count_fragment_simplex column due to error, %s", e)
        exit(1)
    ...
    try:
        ...
        logger.debug("genotype:variants:small_variants:create_duplex_simplex_dataframe:: Successfully reset the index for simplex data frame")
    except:
        e = sys.exc_info()[0]
        logger.error("genotype:variants:small_variants:create_duplex_simplex_dataframe:: Could not reset the index for simplex data frame, due to error, %s", e)
    ...
    df_d.rename(
        ...
        e = sys.exc_info()[0]
        logger.error("genotype:variants:small_variants::create_duplex_simplex_dataframe:: Could not rename column names in duplex data frame due to error, %s", e)
        exit(1)

    try:
        df_d["t_total_count_fragment_duplex"] = df_ds["t_ref_count_fragment_duplex"] + df_ds["t_alt_count_fragment_duplex"]
        logger.debug("genotype:variants:small_variants::create_duplex_simplex_dataframe:: Successfully generated t_total_count_fragment_duplex column")
    except:
        e = sys.exc_info()[0]
        logger.error("genotype:variants:small_variants::create_duplex_simplex_dataframe:: Could not generate t_total_count_fragment_duplex column due to error, %s", e)
        exit(1)
    ...
    df_d["Tumor_Sample_Barcode"] = df_d["Tumor_Sample_Barcode"].str.replace(
        ...
        e = sys.exc_info()[0]
        logger.error("genotype:variants:small_variants:create_duplex_simplex_dataframe:: Could not rename samples in Tumor_Sample_Barcode for duplex data frame, due to error, %s", % e")

    try:
        df_d.set_index("Tumor_Sample_Barcode", append=True, drop=False, inplace=True)
        logger.debug("genotype:variants:small_variants:create_duplex_simplex_dataframe:: Successfully reset the index for duplex data frame")
    except:
        e = sys.exc_info()[0]
        logger.error("genotype:variants:small_variants:create_duplex_simplex_dataframe:: Could not reset the index for duplex data frame, due to error, %s", % e")
    
    # Merge
    try:
        df_ds = df_s.merge(
            df_d[["t_ref_count_fragment_duplex", "t_alt_count_fragment_duplex","t_total_count_fragment_duplex","t_vaf_fragment_duplex"]],
            left_index=True,
            right_index=True,
        )
        logger.info("genotype_variants:small_variants:create_duplex_simplex_dataframe:: Successfully created merge data frame for simplex and duplex data")
    except:
        e = sys.exc_info()[0]
...
try:
    ...
    try:
        ...
        logger.debug("genotype_variants:small_variants:create_duplex_simplex_dataframe:: Successfully renamed samples in Tumor_Sample_Barcode for merged data frame")
    except:
        e = sys.exc_info()[0]
        logger.error("genotype:variants:small_variants:create_duplex_simplex_dataframe:: Could not rename samples in Tumor_Sample_Barcode for merged data frame, due to error, %s", % e")

    try:
        df_ds.set_index(mutation_key, drop=False, inplace=True)
        logger.debug("genotype:variants:small_variants:create_duplex_simplex_dataframe:: Successfully reset the index for merged data frame")
    except:
        e = sys.exc_info()[0]
        logger.error("genotype:variants:small_variants:create_duplex_simplex_dataframe:: Could not reset the index for merged data frame, due to error, %s", % e")
    
    logger.info("Successfully merged data frame and the counts for simplex and duplex MAF")
    return df_ds
