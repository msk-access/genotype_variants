...
class TestGenotype_variants(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mutation_key = ['Chromosome', 'Start_Position', 'End_Position', 'Reference_Allele', 'Tumor_Seq_Allele2']
        self.d_maf = pd.read_csv('tests/test_data/C-100000-L002-d02-DUPLEX_genotyped.maf', sep="\t", header="infer")
        self.s_maf = pd.read_csv('tests/test_data/C-100000-L002-d02-SIMPLEX_genotyped.maf', sep="\t", header="infer")
    ...
    def test_merge_simplex_duplex(self):
        ...
        expected = pd.read_csv('tests/test_data/C-100000-L002-d02-SIMPLEX-DUPLEX_genotyped.maf', sep='\t')
        ...
        assert df_merge.loc[deletion_index]['t_ref_count_fragment_simplex_duplex'] == 537
        assert df_merge.loc[deletion_index]['t_alt_count_fragment_simplex_duplex'] == 0
        assert df_merge.loc[deletion_index]['t_total_count_fragment_simplex_duplex'] == 537
